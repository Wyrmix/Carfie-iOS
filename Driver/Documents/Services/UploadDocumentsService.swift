...
protocol UploadImageDataService {
    func uploadImages(_ imageData: [String: Data], parameters: [String: Any], completion: @escaping (Result<DriverDocument>) -> Void)
}
        ...
        Alamofire.upload(multipartFormData: { (multipartFormData) in
            ...
            case .success(let upload, _, _):
                upload.responseData { response in
                    ...
                    let result: Result<DriverDocument> = decoder.decodeResponse(from: response)
                    completion(result)
                }
