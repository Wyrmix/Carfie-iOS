...
//        viewGoogleRetraction.addGestureRecognizer(gusture)
    ...
    @IBAction func openGoogleMap(){
        ...
        if (UIApplication.shared.canOpenURL(URL(string:"comgooglemaps://")!)) {
            guard let _ = self.pickupLocation,
                  let _ = self.dropLocation,
                  let url = URL(string: "comgooglemaps://?saddr=\(self.slat ?? 0),\(self.sLong ?? 0)&daddr=\(dLat ?? 0),\(dLong ?? 0)&directionsmode=driving"), UIApplication.shared.canOpenURL(url) else { return }
